...
//            }
        ...
        else
        {
            ...
            switch ($tag_sel)
            {
                ...
                case 'insert' :
                    ...
                    $replacement = preg_replace_callback("/(\'\\$[^,]+)/" , function($matcher){return stripslashes(trim($matcher[1],'\'')); },
                    var_export($t, true)); 
                    $out = "<?php \n" . '$k = ' . $replacement . ";\n";
    ...
    function get_val($val)
    {
        ...
        {
            # todo $val = preg_replace("/\[([^\[\]]*)\]/eis", "'.'.str_replace('$','\$','\\1')", $val);
    ...
    function smarty_prefilter_preCompile($source)
    {
        ...
        if ($file_type == '.dwt')
        {
            ...
            # todo $source      = preg_replace($pattern, $replacement, $source);
